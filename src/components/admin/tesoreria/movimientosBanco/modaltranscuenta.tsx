...
export default function ModalCuenta({ abrir, onClose }: ModalProps) {
                ...
                <div className="fixed inset-0  flex items-center justify-center z-50">
                    <div className="  max-w-2/3  max-h-[100vh] rounded shadow bg-white p-8 overflow-y-auto" >
                        <div className="grid grid-rows-2 p-2">
                            <h1 className="text-2xl font-bold text-center mb-2">Nueva Transferencia entre Cuentas</h1>
                        </div>
                        <form action='POST' className='grid grid-cols-2 gap-4 p-4'>
                            ...
                            <TextField
                                id="select-cuenta-destino"
                                select
                                label="Destino"
                                defaultValue=""
                                helperText="Selecciona la cuenta de destino"
                            >
                                {cuentaDestino.map((option) => (
                                    <MenuItem key={option.value} value={option.value}>
                                        {option.label}
                                    </MenuItem>
                                ))}
